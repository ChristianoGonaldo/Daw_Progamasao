...
public class ControladorQuiniela1X2 {
	...
	/**
	 * Permite borrar los datos que se han añadido a lista de altas
	 */
	private void borrado() {
		if(VistaGeneral.pedirConfirmación("Esto que está a punto de hacer es irreversible","¿Está seguro de que desea borrar toda la lista?")) {			
			listaResultados.vaciar();
		}else {
			VistaGeneral.mostrarAviso("Debido a su ineptitud como usuario, vamos a descartar la opción de borrado por el momento");
		}
	}
			...
			if (númElementos == 0) {
				...
				for (String línea : contenido) {
					try {
						...
						VistaGeneral.mostrarError("No se puede importar el archivo debido a un error de: " + e.getLocalizedMessage());
					}
				}
